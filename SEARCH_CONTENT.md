<a id="menu"></a>
# MENU
- 📦 [Lista de Repositórios · Repository List · Repository-Liste](#repository-list)
- 🌳 [Árvore de Tags · Tag Tree · Schlagwortbaum](#tag-tree)
  - 1️⃣ [StageTags](#stage-tags)
  - 2️⃣ [DomainTags](#domain-tags)
  - 3️⃣ [TopicTags](#topic-tags)
  - 4️⃣ [LibraryTags](#library-tags)
  - 5️⃣ [ModuleTags](#module-tags)
- 📚 [Glossário Numérico · Numeric Glossary · Numerisches Glossar](#numeric-glossary)
...
| 👁️ | [DOCS](https://github.com/repository-for-the-curious/DOCS) | 2026-04 | 1.4 | 2.5, 2.6, 2.11 | 3.1, 3.3, 3.5, 3.10, 3.19 | 4.5, 4.9 | 5.6, 5.7 |
| 👁️ | [MODULES](https://github.com/repository-for-the-curious/MODULES) | 2026-04 | 1.4 | 2.5, 2.6, 2.9, 2.11 | 3.1, 3.2, 3.5, 3.10, 3.16, 3.19, 3.20, 3.22, 3.23 | 4.1, 4.4, 4.5, 4.6, 4.9, 4.12 | 5.1, 5.6, 5.7, 5.10 |

## **Apontamento · Pointer · Hinweis:**
- 🇧🇷 Português: Ver [glossário numérico](#numeric-glossary) e o README de cada repositório.
- 🇺🇸 English: See the [numeric glossary](#numeric-glossary) and each repository’s README.
- 🇩🇪 Deutsch: Siehe das [numerische Glossar](#numeric-glossary) und das README jedes Repositories.
...
- **2.2** Process
- **2.3** Compliance
- **2.4** StrategyPlanning
- **2.5** Development
- **2.6** Environment
- **2.7** Solution
- **2.8** UserExperience
- **2.9** DataProcessing
- **2.10** CognitiveBiasMitigation
- **2.11** Analysis
...
- **5.5** evaluate_model
- **5.6** generate_report
- **5.7** call_external_api
- **5.8** authenticate_user
- **5.9** route_request
...
Códigos da [árvore de tags](#tag-tree) com **definição alargada** (contexto da organização ou exemplo concreto de repositório). Para o nome canónico de cada código, continue a usar a árvore.

### 🇺🇸 English

Codes from the [tag tree](#tag-tree) with a **longer gloss** (org context or a concrete repo example). For the canonical label of each code, use the tree.

### 🇩🇪 Deutsch

Codes aus dem [Schlagwortbaum](#tag-tree) mit **ausführlicherer Erläuterung** (Organisationskontext oder konkretes Repo-Beispiel). Der kanonische Name jedes Codes steht im Baum.

---

## Códigos expandidos · Expanded codes · Erweiterte Codes

| Cod | Português | English | Deutsch |
|-----|-----------|---------|---------|
| 3.20 | *Deployment* com **Docker**: subir stack (ex.: base de dados + serviços) em containers, reprodutível entre máquinas | **Docker**-based **deployment**: run stacks (e.g. database + services) in containers, reproducibly across machines | **Deployment** mit **Docker**: Stacks (z. B. Datenbank + Dienste) in Containern, reproduzierbar auf verschiedenen Rechnern |
...
| 4.1 | **pandas**: tabelas, ETL leve, *pipelines* sobre CSV/Parquet e dados em memória | **pandas**: tables, light ETL, pipelines over CSV/Parquet and in-memory data | **pandas**: Tabellen, leichtes ETL, Pipelines über CSV/Parquet und In-Memory-Daten |
| 4.4 | **Selenium**: controlar browser (testes E2E, preenchimento de formulários, scraping com JS) | **Selenium**: drive the browser (E2E tests, form flows, JS-heavy scraping) | **Selenium**: Browser steuern (E2E-Tests, Formulare, Scraping mit JS) |
| 4.5 | **requests**: cliente HTTP em Python (REST, downloads, integração com APIs) | **requests**: Python HTTP client (REST, downloads, API calls) | **requests**: HTTP-Client in Python (REST, Downloads, API-Aufrufe) |
...
**🇧🇷** Este repositório junta **Selenium** (4.4) para automatizar o browser, **pandas** (4.1) para transformar dados, **requests** (4.5) e **BeautifulSoup** (4.6) quando a origem é HTML, **JupyterLab** (4.9) para exploração, e **PostgreSQL** servido em **Docker** — daí **3.20** (*deployment* de stack containerizada) e **2.6** (ambiente reprodutível). **3.16** cobre importação de dados; **3.23** quando o fluxo extrai páginas web. Os módulos autorais **5.1** (carga), **5.6** (relatórios) e **5.7** (API externa) descrevem o tipo de funções esperadas no código.
...
### `my_markdown_tool` — leitura das tags e apontamentos

**🇧🇷** Editor Markdown em TUI com **Textual** (4.13), **Rich** (4.14), **markdown** (4.15) e **PyYAML** (4.16). **1.4** (recurso/ferramenta), **2.5** (desenvolvimento), **2.6** (ambiente Python), **2.11** (análise de uso), **3.1** (implementação), **3.4** (visualização / pré-visualização), **3.21** (lógica central em `markdown_editor.py`). **5.13–5.15**: `render_markdown`, `manage_vault`, `run_terminal_session`. O [README do `my_markdown_tool`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/README.md) declara o mesmo conjunto de tags no frontmatter YAML e resume o produto em PT/EN/DE; [`docs/📝 About.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📝%20About.md) alarga com visão geral, layout, abas/sessão, stack, compatibilidade e índice de `docs/`; [`docs/📘 Funcionalidades.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📘%20Funcionalidades.md) é o canónico técnico (áreas e módulos).

**🇺🇸** TUI Markdown editor using **Textual** (4.13), **Rich** (4.14), **markdown** (4.15), and **PyYAML** (4.16). **1.4** (tool/resource), **2.5** (development), **2.6** (Python environment), **2.11** (usage analysis), **3.1** (implementation), **3.4** (visualization / preview), **3.21** (core logic in `markdown_editor.py`). **5.13–5.15**: `render_markdown`, `manage_vault`, `run_terminal_session`. The [`my_markdown_tool` README](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/README.md) carries the same tag set in YAML frontmatter and a trilingual overview; [`docs/📝 About.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📝%20About.md) expands with layout, tabs/session, stack, compatibility, and a `docs/` index; [`docs/📘 Funcionalidades.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📘%20Funcionalidades.md) is the technical reference.

**🇩🇪** TUI-Markdown-Editor mit **Textual** (4.13), **Rich** (4.14), **markdown** (4.15) und **PyYAML** (4.16). **1.4** (Werkzeug), **2.5** (Entwicklung), **2.6** (Python-Umgebung), **2.11** (Nutzungsanalyse), **3.1** (Implementierung), **3.4** (Visualisierung / Vorschau), **3.21** (Kernlogik in `markdown_editor.py`). **5.13–5.15**: `render_markdown`, `manage_vault`, `run_terminal_session`. Das [README von `my_markdown_tool`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/README.md) enthält dieselben Tags im YAML-Frontmatter und eine dreisprachige Übersicht; [`docs/📝 About.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📝%20About.md) ergänzt Layout, Tabs/Sitzung, Stack, Kompatibilität und einen `docs/`-Index; [`docs/📘 Funcionalidades.md`](https://github.com/repository-for-the-curious/my_markdown_tool/blob/main/docs/📘%20Funcionalidades.md) ist die technische Referenz.
